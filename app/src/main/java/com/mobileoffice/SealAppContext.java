    ...
    private void setInputProvider() {
        ...
        if (moduleList != null) {
            ...
            if (defaultModule != null) {
                ...
                //.getInstance().registerExtensionModule(new SealExtensionModule());
            }
        }
    }
...
//        }
    ...
    public boolean onReceived(Message message, int i) {
        LogUtil.e(TAG, "message:" + message.getConversationType().getName() + "," + message.getContent().getMentionedInfo());
...
//        BroadcastManager.getInstance(mContext).sendBroadcast(GROUP_DISMISS, groupID);
    ...
    public UserInfo getUserInfo(String s) {
        ...
        Log.e(TAG, "testUser:" + s);
        //ToastUtil.showToast("gg:"+s,mContext);
        return null;
    }
    ...
    private void updateGroupTop(final String organSeg) {
        RequestParams params = new RequestParams(URL.RONG);
        params.addBodyParameter("action", "isNewGroup");
        ...
        x.http().get(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                if (photoJson != null && photoJson.getResult() == CONSTS.SEND_OK) {
                    Log.e(TAG, "testGroup:" + photoJson.getMsg());
                    if (photoJson.getMsg().contains("-心脏")) {
                        RongIM.getInstance().refreshGroupInfoCache(new Group(organSeg, photoJson.getMsg(), Uri.parse(URL.TOMCAT_SOCKET+"images/team1.png")));
                    } else if (photoJson.getMsg().contains("-肝脏")) {
                        RongIM.getInstance().refreshGroupInfoCache(new Group(organSeg, photoJson.getMsg(), Uri.parse(URL.TOMCAT_SOCKET+"images/team2.png")));
                    } else if (photoJson.getMsg().contains("-肺")) {
                        RongIM.getInstance().refreshGroupInfoCache(new Group(organSeg, photoJson.getMsg(), Uri.parse(URL.TOMCAT_SOCKET+"images/team3.png")));
                    } else if (photoJson.getMsg().contains("-肾脏")) {
                        RongIM.getInstance().refreshGroupInfoCache(new Group(organSeg, photoJson.getMsg(), Uri.parse(URL.TOMCAT_SOCKET+"images/team4.png")));
                    } else if (photoJson.getMsg().contains("-胰脏")) {
                        RongIM.getInstance().refreshGroupInfoCache(new Group(organSeg, photoJson.getMsg(), Uri.parse(URL.TOMCAT_SOCKET+"images/team5.png")));
                    } else if (photoJson.getMsg().contains("-眼角膜")) {
                        RongIM.getInstance().refreshGroupInfoCache(new Group(organSeg, photoJson.getMsg(), Uri.parse(URL.TOMCAT_SOCKET+"images/team6.png")));
                    } else {
    ...
    public GroupUserInfo getGroupUserInfo(String groupId, String userId) {
        ...
        Log.e(TAG, "testGroupInfo:" + groupId + ",userId:" + userId);
        return null;
    }
...
//        }
    ...
    private void quit(final boolean isKicked) {
        ...
        SharePreUtils.putBoolean("confirm", false, mContext);
        loginActivityIntent.setClass(mContext, LoginActivity.class);
        loginActivityIntent.putExtra("logout", "logout");
